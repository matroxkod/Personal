...
def parse(String description) {
	...
	//Status Polling
        ...
        if(hdcamera == true) {
            ...
            if(body.find("log_text"))
            {
            	log.info("Polled: Log Text")
                sendEvent(name: "currentLog", value: "${device.deviceNetworkId},,${now()},,$body")
            }
            else {
            	/*log.debug ("${device.deviceNetworkId} Parsed $body") 
            	sendEvent(name: "currentLog", value: "${device.deviceNetworkId} || $body")*/
            }
        }
	}
}
